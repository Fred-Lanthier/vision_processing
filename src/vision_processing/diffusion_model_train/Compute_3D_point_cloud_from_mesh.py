...
class RobotMeshLoaderOptimized:
    ...
    def _load_all_meshes(self):
        ...
        print(f"\n✓ Successfully loaded {len(self.mesh_vertices)} meshes")
        print("="*60)
    ...
    def create_point_cloud(self, joint_angles=None, num_points=10000):
        """
        Create point cloud using optimized operations
        ...
        for link_name in self.mesh_vertices.keys():
            if link_name in transforms:
                vertices = self.mesh_vertices[link_name]
                transform = transforms[link_name]
                
                # OPTIMIZATION: Vectorized transform
                # Instead of copying mesh and calling apply_transform,
                # we directly transform vertices with matrix multiplication
                ...
                faces = self.mesh_faces[link_name] + vertex_offset
                all_faces.append(faces)
                
                # Use pre-computed areas
                all_face_areas.append(self.mesh_face_areas[link_name])
        ...
        combined_areas = np.concatenate(all_face_areas)
        
        # Sample points weighted by face area
        points = self._sample_surface_weighted(
            combined_vertices, 
            combined_faces, 
            combined_areas, 
            num_points
...
def benchmark_point_cloud_generation(loader, configurations, num_points=10000):
    ...
    for i, joint_angles in enumerate(configurations):
        ...
        points, colors = loader.create_point_cloud(joint_angles, num_points)
        end_time = time.time()
        
        elapsed = end_time - start_time
        times.append(elapsed)
        all_point_clouds.append((points, colors))
...
def main():
    ...
    with open(config_file, 'r') as file:
        ...
        traj_id = config['trajectory_number']
        write_folder = config['trajectory_folder_write']
        read_folder = config['trajectory_folder_read']
    
    json_file = os.path.join(package_path_fl, write_folder, 
                             f'Trajectory_{traj_id}', f'trajectory_{traj_id}.json')
    
    images_folder = os.path.join(package_path_fl, write_folder,
    ...
    configurations = []
    sample_indices = np.linspace(1, len(states), len(states)).tolist()
