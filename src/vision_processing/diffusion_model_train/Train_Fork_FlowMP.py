"""
Flow Matching Training for Robot Trajectory Prediction with Second-Order Dynamics
================================================================

This integrates velocity and acceleration profiles into the action space (27D)
to enforce physically smooth and dynamically feasible trajectories.
"""
...
# ==============================================================================
# 3. ROBUST NORMALIZER (Handles 9D Obs and 27D Actions)
# ==============================================================================

class Normalizer(nn.Module):
    def __init__(self, obs_dim=9, act_dim=27, stats=None):
        super().__init__()
    ...
    def normalize_obs(self, x): 
        ...
        denom = (self.obs_max - self.obs_min).clamp(min=1e-5)
        return 2 * (x - self.obs_min) / denom - 1

    def normalize_act(self, x): 
        if not self.is_initialized: return x
        denom = (self.act_max - self.act_min).clamp(min=1e-5)
        return 2 * (x - self.act_min) / denom - 1

    def unnormalize_act(self, x):
        if not self.is_initialized: return x
        denom = (self.act_max - self.act_min).clamp(min=1e-5)
        return (x + 1) / 2 * denom + self.act_min
...
class FlowMatchingAgent(nn.Module):
    def __init__(
        ...
        # Robot MLP now explicitly uses obs_dim
        ...
        print(f"  - Action dim (w/ dynamics): {action_dim}")
    ...
    def compute_loss_consistency(self, batch, alpha=0.1):
        ...
        f1 = x_t + (1 - t_expand) * pred_velocity
        f2 = x_t2 + (1 - t2_expand) * pred_velocity2 
        
        # Plus de smooth_loss! Le réseau apprend implicitement la dynamique de 2nd ordre
    ...
    def predict_action(self, obs_dict, num_steps=10, method='euler'):
        ...
        # On dénormalise l'action à 27 dimensions
        action_pred = self.normalizer.unnormalize_act(x)
        
        return {
            # EXTRÊMEMENT IMPORTANT : On retourne au robot uniquement les 9 premières
            # dimensions (Positions + Rotations) à exécuter !
            'action': action_pred[:, :, :9],  
            'action_pred': action_pred
        }


def compute_dataset_stats(dataset, obs_dim=9, act_dim=27):
...
def train_flow_matching(
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_name = "best_fm_model_27D_dynamics_1024.ckpt"
            print("Saved Best EMA Model")
        else:
            save_name = "last_fm_model_27D_dynamics_1024.ckpt"
...
def evaluate_model(model, dataset, device, num_steps=10, num_samples=100):
    ...
    print("\n" + "="*50)
    print("📊 EVALUATION RESULTS (Position Only)")
    print("="*50)
...
def main():
    from Data_Loader_Fork_FlowMP import Robot3DDataset, seed_everything
    
    seed_everything(42)
    
    # === NOUVEAU CONFIG DYNAMIQUES 2ND ORDRE ===
    BATCH_SIZE = 128
    NUM_EPOCHS = 500
    LEARNING_RATE = 1e-4
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    OBS_DIM = 9       # Pos(3) + Rot(6)
    ACTION_DIM = 27   # PosRot(9) + Vel(9) + Acc(9)
    OBS_HORIZON = 2
    PRED_HORIZON = 16
    NUM_POINTS = 1024
    ...
    config = {
        ...
        "lr": LEARNING_RATE,
        "model_name": "FlowMatching_Fork_27D_Dynamics",
        "obs_dim": OBS_DIM,
    ...
    STATS = compute_dataset_stats(train_dataset, obs_dim=OBS_DIM, act_dim=ACTION_DIM)
    ...
    with open(os.path.join(models_dir, 'fm_results_27D.json'), 'w') as f:
