#!/usr/bin/env python3
"""Section 5.3 — Qualité de la génération nominale (Flow Matching).

Produces thesis-styled figures for the FM model using the SHARED thesis_style
module (same fonts/palette/sizing as the SDF and CBF result figures), so 5.3
matches the rest of the mémoire. Three 1x2 panel figures:

  fm_trajectoire_multimodale  — GT vs N stochastic predictions, XY & XZ planes
  fm_erreur_horizon           — position & rotation error along the horizon
                                (median + IQR + 10-90 bands over the val set)
  fm_distribution_erreur      — ADE and final-rotation-error histograms

Palette convention (thesis_style):
  BLUE  = model under test (predictions, hero)
  GREY  = reference (ground truth)
  ORANGE= history / median accents

Run with the venv python from this directory:
    python3 generate_fm_thesis_figures.py
Figures are written to ./thesis_fm_figures/ as .svg (thesis) + .png (preview).
"""
import os
import sys
import time
...
# 9D dynamics model: trajectories are expressed in a fork-tip-centered frame
# (translation-only centering, world orientation preserved). GT actions and
# model predictions both live in this same local frame, so the comparison
# figures below need no world-frame transform — the offset would be identical
# for both curves. Deployment re-adds the fork-tip world position downstream.
...
def compute_dtw_position_error(pred_pos, gt_pos, normalize=True):
    """Normalized DTW distance between two 3D position trajectories.

    DTW warps the two sequences with an optimal many-to-many alignment, so it
    measures path-SHAPE fidelity independent of timing/phase — unlike the
    per-step point-to-point distance used along the horizon. It is a single
    scalar per trajectory (cannot be resolved per horizon step). Same metric
    used as the success criterion in generate_results_Fork_FM.py."""
...
def fig_trajectory_multimodality(model, sample, device, idx,
    ...
    ts.save(fig, OUTDIR, 'fm_trajectoire_multimodale')


# ==============================================================================
# 5.3.2 — Erreur le long de l'horizon (cohérence avec la cible)
# ==============================================================================
...
def fig_error_along_horizon(model, dataset, device,
    ...
    print(f"[5.3] erreur le long de l'horizon "
    ...
    for data, ylab, name in [
        (pos_errs, 'Erreur de position (cm)', 'fm_erreur_horizon_position'),
        (rot_errs, 'Erreur de rotation ($^\\circ$)', 'fm_erreur_horizon_rotation'),
    ]:
...
# ==============================================================================
# 5.3.2 — Distributions d'erreur sur l'ensemble de validation
# ==============================================================================
...
def fig_error_distributions(model, dataset, device,
                            ...
                            num_steps=NUM_STEPS):
    n_windows = min(n_windows, len(dataset))
    print(f"[5.3] distributions d'erreur "
          f"({n_windows} fenêtres x {n_samples} prédictions) ...")
    idxs = np.linspace(0, len(dataset) - 1, n_windows, dtype=int)
    dtw_err, rot_final = [], []
    for idx in idxs:
    ...
    for data, xlab, name in [
        (dtw_err, 'DTW de position (cm)', 'fm_distribution_erreur_dtw'),
        (rot_final, 'Erreur de rotation finale ($^\\circ$)',
         'fm_distribution_erreur_rotation'),
    ]:
        fig, ax = plt.subplots(figsize=(ts.width(0.5), 3.3))
...
def fig_loss_curves(history, outdir=OUTDIR, name='fm_loss_curves'):
    ...
    # Raw (faint) + EMA (bold), same semantic colours as the rest of the mémoire.
...
def main():
    ...
    data_path = os.path.join(DATA_ROOT, 'Trajectories_preprocess')
    ...
    # Normalization stats MUST match the checkpoint: positions are predicted in
    # the normalized [-1, 1] space and only become metric after unnormalize_act.
    ...
    # Annexe I — figures de convergence / d'ablation du modèle FM.
