...
# Import Flow Matching model and data loader (PICK-AND-PLACE variant)
from Train_PickPlace import (
    FlowMatchingAgent,
    Normalizer,
...
from Data_Loader_PickPlace import Robot3DDataset
...
# Prefixe les figures pour ne pas ecraser celles de la fourchette.
PREFIX = 'pp_'
DEFAULT_CKPT_NAME = 'best_fm_model_pickplace_10D_1024.ckpt'
...
def infer_single(model, sample, DEVICE, num_steps=10, method='euler'):
    ...
    with torch.no_grad():
        ...
        pred_action = result['action_pred'].cpu().numpy()[0] # Full 10D prediction

    inference_time = time.perf_counter() - start_time

    gt_action = sample['action'].numpy() # Full 10D Ground Truth
...
def plot_training_history(history):
    ...
    ax.set_yscale('log')
    ax.set_title('Flow Matching (PickPlace 10D) — Training Convergence')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('MSE Loss (log scale)')
    ax.legend()
    ax.grid(True, which='both')
    fig.savefig(f'{PREFIX}01_fm_train_history_pp.png')
    plt.close(fig)
...
def plot_3d_multimodality_with_orientation(model, sample, DEVICE, idx, n_samples=20, num_steps=10):
    ...
    fig.savefig(f'{PREFIX}02_fm_3d_spaghetti_seq{idx}_pp.png')
    plt.close(fig)
...
def plot_multimodality_spaghetti_2d(model, sample, DEVICE, idx, n_samples=50, num_steps=10):
    ...
    fig.savefig(f'{PREFIX}02_fm_spaghetti_2d_seq{idx}_pp.png')
    plt.close(fig)
...
def plot_rotation_error_over_time(gt_action, pred_action, idx):
    ...
    ax.legend()
    ax.grid(True)
    fig.savefig(f'{PREFIX}05_fm_rotation_error_seq{idx}_pp.png')
    plt.close(fig)


def plot_gripper_prediction(gt_action, pred_action, idx):
    if gt_action.shape[-1] <= 9 or pred_action.shape[-1] <= 9:
        return

    fig, ax = plt.subplots(figsize=(9, 4.5))
    t = np.arange(len(gt_action))
    pred_grip = np.clip(pred_action[:, 9], 0.0, 1.0)
    gt_grip = gt_action[:, 9]

    ax.step(t, gt_grip, where='post', color=STYLE['gt_color'], linewidth=STYLE['gt_lw'],
            label='Ground Truth')
    ax.plot(t, pred_grip, color=STYLE['pred_color'], marker='o', markersize=4,
            linewidth=STYLE['pred_lw'], label='Prediction')
    ax.axhline(y=GRIPPER_BINARY_THRESHOLD, color=STYLE['accent_color'], linestyle='--', linewidth=1.2,
               label='Open/close threshold')

    ax.set_title(f'Seq {idx} — Gripper Open Prediction')
    ax.set_xlabel('Timestep')
    ax.set_ylabel('gripper_open')
    ax.set_ylim(-0.1, 1.1)
    ax.legend()
    ax.grid(True)
    fig.savefig(f'{PREFIX}05b_fm_gripper_seq{idx}_pp.png')
    plt.close(fig)
...
def make_flow_matching_gif(model, sample, DEVICE, idx, num_steps=10):
    ...
    ani.save(f'{PREFIX}03_fm_integration_seq{idx}.gif', writer='pillow')
    plt.close(fig)
...
def run_quantitative_analysis(model, val_dataset, DEVICE, n_samples=20, num_steps=10):
    ...
    for idx in tqdm(indices):
        ...
        has_gripper = gt_action.shape[-1] > 9
        gt_grip = gt_action[:, 9] if has_gripper else None

        seq_ades, seq_angles, seq_preds = [], [], []
        seq_grip_mae, seq_grip_acc = [], []
        ...
        for _ in range(n_samples):
            ...
            if has_gripper and pred.shape[-1] > 9:
                pred_grip = np.clip(pred[:, 9], 0.0, 1.0)
                seq_grip_mae.append(np.mean(np.abs(pred_grip - gt_grip)))
                seq_grip_acc.append(np.mean(
                    (pred_grip >= GRIPPER_BINARY_THRESHOLD)
                    == (gt_grip >= GRIPPER_BINARY_THRESHOLD)
                ))
        ...
        if seq_grip_mae:
            row['GripperMAE'] = float(np.mean(seq_grip_mae))
            row['GripperAcc'] = float(np.mean(seq_grip_acc) * 100.0)
        results.append(row)
    ...
    if 'GripperMAE' in df:
        print(f"  Mean Gripper MAE     : {df['GripperMAE'].mean():.3f}")
        print(f"  Mean Gripper Acc     : {df['GripperAcc'].mean():.2f} %")
    print("=" * 60)
    return df
...
def plot_error_distributions(df):
    ...
    fig.tight_layout()
    fig.savefig(f'{PREFIX}04_fm_metrics_distribution_pp.png')
    plt.close(fig)
...
def run_inference_speed_benchmark(model, val_dataset, DEVICE,
    ...
    # (xlim auto : depend du GPU, pas de plage codee en dur)
    ...
    fig.tight_layout()
    fig.savefig(f'{PREFIX}06_fm_inference_speed_pp.png')
    plt.close(fig)
...
def main():
    ...
    ckpt_path = os.path.join(pkg_path, 'models', DEFAULT_CKPT_NAME)
    if not os.path.exists(ckpt_path):
        fallback = os.path.join(pkg_path, 'models', 'last_fm_model_pickplace_10D_1024.ckpt')
    ...
    OBS_DIM = int(cfg.get('obs_dim', 10))
    ...
    print(
        ...
        f"obs_dim={OBS_DIM} action_dim={ACTION_DIM} horizon={PRED_HORIZON}"
    )

    val_dataset = Robot3DDataset(
        data_path, mode='val', val_ratio=0.2, seed=42,
        num_points=NUM_POINTS, obs_horizon=OBS_HORIZON,
        pred_horizon=PRED_HORIZON, data_source='all')
    print(f"✅ Validation set: {len(val_dataset)} sequences")
    if len(val_dataset) == 0:
        print("❌ Validation set is empty. Run Data_preprocess_PickPlace.py after collecting data.")
        return
    ...
    if len(val_dataset) > sample_idx:
        ...
        pred_action, _, _, _ = infer_single(model, sample, DEVICE, num_steps=NUM_STEPS)
        plot_gripper_prediction(sample['action'].numpy(), pred_action, sample_idx)
    ...
    plot_gripper_prediction(worst_sample['action'].numpy(), pred_worst, idx_worst)
