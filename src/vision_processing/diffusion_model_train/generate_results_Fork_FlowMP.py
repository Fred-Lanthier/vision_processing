...
from Train_Fork_FlowMP import FlowMatchingAgent, Normalizer, compute_dataset_stats, custom_collate_fn
from Data_Loader_Fork_FlowMP import Robot3DDataset
...
def infer_single(model, sample, DEVICE, num_steps=10, method='euler'):
    ...
    with torch.no_grad():
        ...
        pred_action = result['action_pred'].cpu().numpy()[0] # Full 27D prediction

    inference_time = time.perf_counter() - start_time

    gt_action = sample['action'].numpy() # Full 27D Ground Truth
...
def infer_with_intermediate_steps(model, sample, DEVICE, num_steps=10):
    ...
    with torch.no_grad():
        ...
        B = 1
        # Noise is generated in the 27D space
...
def plot_dynamics_profiles(gt_action, pred_action, idx):
    print(f"📈 Generating dynamics profiles plot for seq {idx}...")
    
    # Extract magnitudes for Ground Truth
    gt_vel_pos_mag = np.linalg.norm(gt_action[:, 9:12], axis=1)
    gt_vel_rot_mag = np.linalg.norm(gt_action[:, 12:18], axis=1) # Proxy pour vitesse angulaire
    gt_acc_pos_mag = np.linalg.norm(gt_action[:, 18:21], axis=1)
    
    # Extract magnitudes for Prediction
    pred_vel_pos_mag = np.linalg.norm(pred_action[:, 9:12], axis=1)
    pred_vel_rot_mag = np.linalg.norm(pred_action[:, 12:18], axis=1)
    pred_acc_pos_mag = np.linalg.norm(pred_action[:, 18:21], axis=1)
    
    timesteps = np.arange(len(gt_action))
    
    fig, axs = plt.subplots(2, 2, figsize=(14, 8))
    
    # Vitesse Translation
    axs[0, 0].plot(timesteps, gt_vel_pos_mag, color=STYLE['gt_color'], lw=2, label='GT Translation Vel')
    axs[0, 0].plot(timesteps, pred_vel_pos_mag, color=STYLE['pred_color'], lw=2, label='Pred Translation Vel')
    axs[0, 0].set_title("Translation Velocity Magnitude")
    axs[0, 0].grid(True)
    axs[0, 0].legend()
    
    # Vitesse Rotation (Coup de poignet)
    axs[0, 1].plot(timesteps, gt_vel_rot_mag, color=STYLE['gt_color'], lw=2, label='GT Rotation Vel (6D)')
    axs[0, 1].plot(timesteps, pred_vel_rot_mag, color=STYLE['pred_color'], lw=2, label='Pred Rotation Vel (6D)')
    axs[0, 1].set_title("Angular Velocity Proxy (Coup de poignet)")
    axs[0, 1].grid(True)
    axs[0, 1].legend()

    # Accélération Translation
    axs[1, 0].plot(timesteps, gt_acc_pos_mag, color=STYLE['gt_color'], lw=2, label='GT Translation Acc')
    axs[1, 0].plot(timesteps, pred_acc_pos_mag, color=STYLE['pred_color'], lw=2, label='Pred Translation Acc')
    axs[1, 0].set_title("Translation Acceleration Magnitude")
    axs[1, 0].grid(True)
    axs[1, 0].legend()
    ...
    fig.tight_layout()
    fig.savefig(f'07_fm_dynamics_profiles_seq{idx}.png')
    plt.close(fig)
...
def main():
    ...
    ckpt_path = os.path.join(pkg_path, 'models', 'last_fm_model_27D_dynamics_1024.ckpt')
    ...
    if len(val_dataset) > sample_idx:
        ...
        pred_action, _, _, _ = infer_single(model, sample, DEVICE, num_steps=NUM_STEPS)
        plot_dynamics_profiles(sample['action'].numpy(), pred_action, sample_idx)
    ...
    plot_dynamics_profiles(worst_sample['action'].numpy(), pred_worst, idx_worst)
