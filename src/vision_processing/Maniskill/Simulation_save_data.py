...
import rospkg  # Pour trouver ton rospack\
import time
...
def save_states_to_json(trajectory_path, trajectory_id, states, current_task, food_type, plate_type, camera_pose):
    # Note: On sauvegarde dans trajectory_path (Trajectory_X) pour avoir trajectory_X.json au bon endroit
...
class FoodSpikingEnv(PickCubeEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        # self.target_shape_type = np.random.choice(["sausage"])
...
if __name__ == "__main__":
    ...
    for ep in range(NUM_EPISODES):
        ...
        # 1. Création de l'arborescence pour CET épisode
        traj_id = get_next_trajectory_id([BASE_RECORD_DIR, BASE_PREPROCESS_DIR])
        traj_path, img_dir = create_trajectory_directories(BASE_RECORD_DIR, traj_id)
        
        # 2. Capture des poses initiales des caméras par rapport à la base du robot
        base_pose = robot.pose  # Pose de la base du Franka
        cam_static_pose = links["camera_static_link"].pose
        cam_wrist_pose = links["camera_wrist_link"].pose
        
        # Mathématiques SAPIEN : Pose Relative = Base^(-1) * Camera
        rel_static_pose = base_pose.inv() * cam_static_pose
        rel_wrist_pose = base_pose.inv() * cam_wrist_pose
        
        # Conversion en matrices 4x4 (listes de listes pour JSON)
        camera_poses = {
            "T_static_s": rel_static_pose.to_transformation_matrix().tolist(),
            "T_ee_s": rel_wrist_pose.to_transformation_matrix().tolist()
        }
        ...
        if uw.target_shape_type == "broccoli":
        ...
        while not done:
            # --- LECTURE DE L'ÉTAT (Nécessaire pour le contrôle) ---
            links = {l.name: l for l in uw.agent.robot.get_links()}
            fork_tip = links["fork_tip"]
            time_step = step_count * uw.control_timestep
            
            # =========================================================
            # FILTRE DE FRÉQUENCE D'ENREGISTREMENT (Mathématiques Entières)
            # =========================================================
            # Si le reste de la division entière est 0, c'est le moment d'enregistrer !
            if step_count % steps_per_record == 0:
                
                # --- SAUVEGARDE DES IMAGES SUR LE DISQUE ---
                sensors = obs["sensor_data"]
                
                rgb_static_buffer.append(sensors["static"]["rgb"][0].cpu().numpy())
                depth_static_buffer.append(sensors["static"]["depth"][0].cpu().numpy())
                
                rgb_ee_buffer.append(sensors["ee"]["rgb"][0].cpu().numpy())
                depth_ee_buffer.append(sensors["ee"]["depth"][0].cpu().numpy())

                # --- PRÉPARATION DES CLÉS JSON POUR LES CAMÉRAS ---
                camera_static_info = {
                    "rgb": f"static_rgb_step_{record_step_count:04d}.png",
                    "depth": None,
                    "depth_npy": f"static_depth_step_{record_step_count:04d}.npy"
                }
                camera_ee_info = {
                    "rgb": f"ee_rgb_step_{record_step_count:04d}.png",
                    "depth": None,
                    "depth_npy": f"ee_depth_step_{record_step_count:04d}.npy"
                }

                # --- SAUVEGARDE DE L'ÉTAT DANS LA LISTE JSON ---
                states.append({
                    "step_number": record_step_count,  # 0, 1, 2, 3... (séquentiel parfait)
                    "simulation_step": step_count,     # Optionnel : garde une trace du vrai pas de simulation
                    "time_step": round(time_step, 4),  # 0.0, 0.1, 0.2... (parfait)
                    "joint_positions": robot.get_qpos()[0].cpu().numpy().tolist(),
                    "joint_velocities": robot.get_qvel()[0].cpu().numpy().tolist(),
                    "end_effector_position": fork_tip.pose.p[0].cpu().numpy().tolist(),
                    "end_effector_orientation": fork_tip.pose.q[0].cpu().numpy().tolist(),
                    "end_effector_velocity": fork_tip.linear_velocity[0].cpu().numpy().tolist() + 
                                             fork_tip.angular_velocity[0].cpu().numpy().tolist(),
                    "phase": current_phase,
                    "camera_static": camera_static_info,
                    "camera_ee": camera_ee_info
                })
                
                # On incrémente notre compteur d'enregistrement !
                record_step_count += 1
            # =========================================================
        ...
        # --- FIN DE L'ÉPISODE ---
        print(f"    Épisode terminé. Écriture de {len(rgb_static_buffer)} frames sur le disque...")
        
        # Vidage des tampons RAM vers le disque
        for i in range(len(rgb_static_buffer)):
            # Caméra Statique
            Image.fromarray(rgb_static_buffer[i]).save(os.path.join(img_dir, f"static_rgb_step_{i:04d}.png"))
            np.save(os.path.join(img_dir, f"static_depth_step_{i:04d}.npy"), depth_static_buffer[i])
            
            # Caméra Poignet (EE)
            Image.fromarray(rgb_ee_buffer[i]).save(os.path.join(img_dir, f"ee_rgb_step_{i:04d}.png"))
            np.save(os.path.join(img_dir, f"ee_depth_step_{i:04d}.npy"), depth_ee_buffer[i])
        ...
        save_states_to_json(
            ...
            food_type=uw.target_shape_type,
            plate_type="pedestal",
            camera_pose=camera_poses
        )
    end = time.time()
    print(f"Data collection completed in {(end - starts):.3f} secondes.")
    env.close()