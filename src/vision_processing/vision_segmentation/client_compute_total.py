...
def main():
    ...
    if result and result.get('success'):
        print("\n✅ RÉSULTATS :")
        print(f"   ⏱️ Temps total : {result.get('total_computation_time', 0):.2f}s")
        
        items = result['food_items']
        if not items:
            print("   ⚠️ Aucun aliment détecté.")
        else:
            for item in items:
                print(f"   🍽️  Aliment : {item['name']}")
                print(f"      🔢 Nombre : {item['count']}")
                # print(f"      🎯 Centroïdes : {item['centroids']}")
                # print(f"      📐 Aires : {item['areas']}")
                print(f"      ⏱️ Temps : {item['computation_time']:.2f}s")
        
        # --- Sauvegarde des deux images ---
        print("\n💾 Sauvegarde des images résultats...")
        save_base64_image(result.get('overlay_base64'), "images/client_resultat_overlay.png")
        save_base64_image(result.get('rainbow_base64'), "images/client_result_rainbow_map.png")
        print("Done.")
