...
class GenerateSDF:
    """
    Pipeline to create a volumetric SDF from a depth map with ASYMMETRIC safety zones.
    
    Uses mesh face normals (oriented toward camera) to distinguish:
    - Normal side (cavity / camera side): thin forbidden zone (inner_radius)
    - Outer side (under / sides of bowl):  massive forbidden zone (outer_radius)
    
    Math: 
        side = sign( dot(query - closest_surface_point, face_normal) )
        
        Normal side:  SDF(x) = distance - inner_radius   (e.g. 2mm)
        Outer side:   SDF(x) = distance - outer_radius   (e.g. 1m)
    
    Result:
        SDF < 0 : Forbidden zone (collision or outer side)
        SDF = 0 : On the virtual boundary
        SDF > 0 : Safe (can approach)
    """
    ...
    def __init__(self, depth_map_complete=None, image_path=None, intrinsics=None):
        ...
        self.inner_radius = 0.0    # Cavity side radius
        self.outer_radius = 0.0    # Under/sides radius
    ...
    def run_pipeline(self, object_list, inner_radius_mm=2.0, outer_radius_mm=100.0,
                     target_faces=10000, sdf_resolution=250,
                     save_meshes=True, output_dir="."):
        """
        Run the pipeline: Depth -> Surface -> Asymmetric SDF
        
        Args:
            inner_radius_mm: Safety buffer on the NORMAL side (cavity/camera side).
                             Small value (1-2mm) to allow robot approach from inside.
            outer_radius_mm: Safety buffer on the OUTER side (under/sides of bowl).
                             Large value (e.g. 1000mm = 1m) to block approach 
                             from below and sides.
        """
        print("=" * 60)
        print(f"SDF Generation (Asymmetric Mode)")
        print(f"   Inner radius (cavity side):  {inner_radius_mm} mm")
        print(f"   Outer radius (under/sides):  {outer_radius_mm} mm")
        print("=" * 60)
        ...
        # Step 6: Compute Asymmetric SDF
        self._compute_sdf(resolution=sdf_resolution, 
    ...
    def process_object_list(self, object_list):
        ...
        print(f" Analyse de la liste : {object_list}")
        print("-" * 40)
            ...
            try:
                ...
                if masks is not None:
                    ...
                    if masks.size == 0 or not np.any(masks):
                        print(f"      Aucun pixel trouvé pour '{object_name}'")
                        continue
                    ...
                    print(f"      Retour vide pour '{object_name}'")
    ...
    def _pca_align(self):
        ...
        # Sort: X=largest spread, Y=medium, Z=thinnest (plate thickness)
        ...
        # 4. FIX ORIENTATION: ensure the camera-facing side (cavity) faces +Z
        up_direction = -centroid  # points from plate toward camera
        up_after_pca = up_direction @ R  # rotate with same PCA transform
        
        if up_after_pca[2] < 0:
            aligned_points[:, 2] = -aligned_points[:, 2]
            print("   Flipped Z to ensure cavity faces up.")
        
        # 5. Place on floor (Z=0)
    ...
    def _compute_sdf(self, resolution=150, margin=0.12, inner_radius_mm=2.0, outer_radius_mm=100.0):

        print(f"\n[3/3] Computing Asymmetric SDF (resolution={resolution})")
        print(f"   Inner radius (cavity side):  {inner_radius_mm} mm")
        print(f"   Outer radius (under/sides):  {outer_radius_mm} mm")
        
        self.inner_radius = inner_radius_mm / 1000.0  # Convert to meters
        self.outer_radius = outer_radius_mm / 1000.0
        # Keep virtual_radius as inner for backward compatibility
        self.virtual_radius = self.inner_radius
        ...
        try:
            ...
            n_points = int(np.prod(self.n_voxels))
            ...
            # ---- Open3D RaycastingScene: uses Intel Embree BVH (extremely fast) ----
            # Returns closest points + triangle IDs for millions of points in seconds.
            # Open3D is already in our stack (used for Poisson reconstruction).
            import open3d.core as o3c
            
            # Convert trimesh → Open3D tensor mesh
            mesh_t = o3d.t.geometry.TriangleMesh()
            mesh_t.vertex.positions = o3c.Tensor(mesh.vertices.astype(np.float32))
            mesh_t.triangle.indices = o3c.Tensor(mesh.faces.astype(np.int32))
            
            scene = o3d.t.geometry.RaycastingScene()
            scene.add_triangles(mesh_t)
            
            # One call: closest points + triangle IDs for ALL grid points
            query = o3c.Tensor(all_points.astype(np.float32))
            result = scene.compute_closest_points(query)
            
            closest_points = result['points'].numpy()       # (N, 3)
            triangle_ids = result['primitive_ids'].numpy()   # (N,)
            
            max_z = np.max(mesh.vertices[:, 2])

            # Side detection: dot(query - closest, face_normal_at_triangle)
            # Normals point toward camera = into cavity (from orient_normals_towards_camera)
            to_query = all_points - closest_points
            distances = np.linalg.norm(to_query, axis=1)
            
            face_normals = mesh.face_normals[triangle_ids]
            dots = np.sum(to_query * face_normals, axis=1)
            
            # True = côté cavité (au-dessus), False = côté extérieur (en-dessous/dedans)
            is_normal_side = dots > 0  

            Z_flat = all_points[:, 2] # Récupérer les coordonnées Z de tous les points
            is_above_container = Z_flat > (max_z + 0.002)
            
            # Forcer is_normal_side à True si on est au-dessus
            is_normal_side = is_normal_side | is_above_container

            # CRITIQUE : Rendre la distance négative si on est du mauvais côté (intérieur/dessous)
            signed_distances = np.where(is_normal_side, distances, -distances)

            # Appliquer les rayons asymétriques
            radius = np.where(is_normal_side, self.inner_radius, self.outer_radius)

            # Maintenant, l'intérieur sera strictement négatif, mais le plafond sera dégagé !
            final_sdf = signed_distances - radius
            
            elapsed = time.time() - t_start
            
            # Stats
            n_normal = np.sum(is_normal_side)
            n_outer = n_points - n_normal
            print(f"   Computed in {elapsed:.2f}s")
            print(f"   Normal side (cavity): {n_normal:,} points ({100*n_normal/n_points:.1f}%)")
            print(f"   Outer side (under):   {n_outer:,} points ({100*n_outer/n_points:.1f}%)")
            print(f"   SDF range: [{final_sdf.min():.4f}, {final_sdf.max():.4f}]")
    ...
    def load(self, path):
        ...
        self.inner_radius = float(data['inner_radius']) if 'inner_radius' in data else self.virtual_radius
        self.outer_radius = float(data['outer_radius']) if 'outer_radius' in data else self.virtual_radius
...
if __name__ == "__main__":
    ...
    if not os.path.exists(npz_file):
        ...
        sdf = GenerateSDF.from_file(npz_file)