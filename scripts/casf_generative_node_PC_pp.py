...
#   - Train_PickPlace.py          → same obs/action normalizer, but 10D
#                                   pose9 + gripper_open.
...
#   FlowMatchingAgentPickPlace → 10D PickPlace Normalizer
...
from Train_PickPlace import FlowMatchingAgent as FlowMatchingAgentPickPlace
...
class CASFGenerativeNode:
    def __init__(self):
        rospy.init_node('casf_generative_node_PC_pp')
        ...
        model_name = rospy.get_param("~model_name", "last_fm_model_pickplace_10D_1024.ckpt")
        ...
        self.is_pickplace_10d = self.action_dim == 10 or self.obs_dim == 10
        if self.is_pickplace_10d:
            AgentCls = FlowMatchingAgentPickPlace
        elif self.norm_is_9d:
            AgentCls = FlowMatchingAgent9D
        else:
            AgentCls = FlowMatchingAgentFM
        rospy.loginfo(
            "🧭 Detected %s checkpoint %s",
            "PickPlace 10D pose+gripper"
            if self.is_pickplace_10d
            else ("9D-dynamics (obs/act)" if self.norm_is_9d else "FM (shared pos)"),
            model_name,
        )
        ...
        if self.norm_is_9d or self.is_pickplace_10d:
    ...
    def _setup_fm_compile(self):
        ...
        try:
            ...
            dummy_obs = {
                ...
                'agent_pos': torch.zeros(B, self.obs_horizon, self.obs_dim, device=self.device),
            }
    ...
    def joint_callback(self, msg):
        try:
            ...
            if all(n in pos_dict for n in finger_names):
                opening = 0.5 * (float(pos_dict[finger_names[0]]) + float(pos_dict[finger_names[1]]))
                self.current_gripper_open = 1.0 if opening >= self.gripper_open_threshold else 0.0
            q_list = []
    ...
    def control_loop(self, event):
        ...
        if self.obs_dim > 9:
            curr_obs = np.concatenate([
                curr_pose_9d,
                np.array([self.current_gripper_open], dtype=np.float32),
            ]).astype(np.float32)
        else:
            curr_obs = curr_pose_9d
        
        self.obs_queue.append(curr_obs)
    ...
    def generate_safe_trajectory(self, obs_dict, curr_pos, P_obs, q_source, lam=1e-5):
        ...
        if self.enable_casf_correction and P_obs is not None and P_obs.shape[0] > 0:
            ...
            for i in range(N_CORR if P_obs_filtered.shape[0] > 0 else 0):
                ...
                se3_fork_gpu = decode_9d_to_se3_gpu(A_world[..., :9].reshape(H, 9))
                ...
                with torch.no_grad():
                    ...
                    v_world = (A_next_world - A_world).reshape(B, H, self.action_dim)
                    ...
                    # CASF only warps Cartesian position. Preserve rotation and
                    # gripper dimensions from the current flow state.
