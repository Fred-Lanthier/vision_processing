...
class FlowMatchingInferenceNodeFork:
    def __init__(self):
        ...
        # =====================================================
        # COMMIT MODE — open-loop final approach
        # =====================================================
        self.COMMIT_DISTANCE = rospy.get_param("~commit_distance", 0.025)  # 25 mm
        self.is_committed = False
        self.committed_trajectory = None
        self.commit_step = 0
        self.fork_food_distance = float('inf')  # Updated by condition node
    ...
    def cloud_callback(self, msg):
        # Direct numpy read — ~100x faster than list(pc2.read_points(...))
        # Works because condition node publishes xyz32 (3 × float32, tightly packed)
        if len(msg.data) == 0: return
        points = np.frombuffer(msg.data, dtype=np.float32).reshape(-1, 3).copy()
        
        # Filter NaN (cheap safety check)
    ...
    def control_loop(self, event):
        ...
        # =====================================================
        # COMMIT MODE — execute remaining trajectory open-loop
        # =====================================================
        if self.is_committed:
            remaining = self.committed_trajectory[self.commit_step:]
            if len(remaining) == 0:
                # Trajectory fully executed — exit commit mode
                rospy.loginfo("✅ Committed trajectory completed")
                self.is_committed = False
                self.committed_trajectory = None
                return
            
            # Publish remaining steps (trajectory follower will track them)
            self.publish_trajectory(remaining)
            # Advance by 1 step per control cycle
            self.commit_step += 1
            return

        # =====================================================
        # NORMAL MODE — closed-loop inference
        # =====================================================
        ...
        # Gram-Schmidt orthogonalization (vectorized over all timesteps)
        col1 = action_centered_np[:, 3:6].copy()
        col2 = action_centered_np[:, 6:9].copy()
        col1 /= (np.linalg.norm(col1, axis=1, keepdims=True) + 1e-8)
        col2 -= np.sum(col2 * col1, axis=1, keepdims=True) * col1
        col2 /= (np.linalg.norm(col2, axis=1, keepdims=True) + 1e-8)
        action_centered_np[:, 3:6] = col1
        action_centered_np[:, 6:9] = col2
        ...
        # =====================================================
        # CHECK: should we commit to this trajectory?
        # =====================================================
        if self.fork_food_distance < self.COMMIT_DISTANCE:
            # COMMIT: publish full trajectory, stop re-planning
            rospy.loginfo(f"🔒 COMMIT MODE (d={self.fork_food_distance*1000:.1f}mm) — executing open-loop")
            self.is_committed = True
            self.committed_trajectory = action_world_np  # Full pred_horizon
            self.commit_step = 0
            self.publish_trajectory(action_world_np)
            return

        # ── Normal ensembled output ──
