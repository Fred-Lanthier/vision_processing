...
# ── 1. ANTI CPU-THRASHING (À METTRE TOUT EN HAUT) ──
import os
...
class EnvironmentGridNP:
    ...
    def clear_robot(self, points, margin=0.02):
        ...
        # ── OPTIMISATION 3: Fini le np.unique() ici, on assigne directement ──
...
# =====================================================================
#  CPU UNPROJECT (OPTIMISÉ)
# =====================================================================

def cpu_unproject(depth_np, T_np, fx, fy, cx, cy, valid_mask=None, z_min=0.01, z_max=1.5):
    """
    Déprojette l'image. Si valid_mask est fourni, on ignore complètement
    les calculs pour les pixels hors du masque (gain de performance énorme).
    """
...
def cpu_unproject_strided(depth_np, T_np, fx, fy, cx, cy,
    ...
    """
    Unproject a subsampled depth image for global scene awareness.

    Instead of processing all 480x640 = 307k pixels, we take every
    Nth pixel (stride=4 → ~19k pixels). The 3D points are coarser
    but enough to detect obstacles anywhere in the camera's field.

    Args:
        stride: take every Nth pixel in both rows and columns.
                stride=4 means 16x fewer pixels → ~1-2ms on CPU.
    """
    ...
    # Scale pixel coords back to full-resolution image coordinates
...
class SDFNode:
    def __init__(self):
        ...
        # ── Container reference cloud (ICP-tracked, occlusion-resistant) ──
        ...
        rospy.loginfo("SDF Node READY (CPU-only, no GPU, Highly Optimized)")
    ...
    def _container_cloud_cb(self, msg):
        """Receive ICP-tracked container cloud from segmentation_node."""
        pts = np.array(
    ...
    def _depth_cb(self, depth_msg):
        # ── OPTIMISATION 1: L'étranglement de fréquence (throttle) a été retiré ──
    ...
    # ==================================================================
    #  OPTIMISATION 2: CALCUL DU MASQUE 2D ROI
    # ==================================================================
    def _get_roi_mask(self, depth_np, T_world_cam):
        """
        Projette les coins de la BBox 3D du conteneur sur la caméra pour 
        créer un masque 2D. Ainsi on ne calcule la matrice inverse que 
        pour les quelques pixels à l'intérieur de la boîte !
        """
        ...
        # Les 8 coins de la bounding box en 3D
        corners = np.array([
        ...
        # Passage du repère World au repère Caméra
        ...
        if not np.any(valid_z):
            return None # La boîte est derrière la caméra
        
        # On détermine les limites Z pour filtrer la profondeur
        z_min = max(0.01, z_cam.min() - 0.05) # Marge de 5cm
        z_max = z_cam.max() + 0.05
        
        # Projection en 2D pour trouver les limites U,V dans l'image
    ...
    def _bg_loop(self):
        while self._running and not rospy.is_shutdown():
            ...
            try:
                ...
                # ── 1a. Points de la zone du conteneur (Haute densité via masque ROI) ──
                ...
                # ── 1b. Full scene points (low density) ──
                full_obs = cpu_unproject_strided(
                    z, T, self.fx, self.fy, self.cx, self.cy,
                    stride=4, z_min=0.01, z_max=3.0 # z_max=3.0 allows you to see the room
                )
                ...
                # 2a. Integrate Container-area ONLY
                ...
                # 2b. Filter target out of full_obs (BUT DO NOT INTEGRATE INTO GRID)
                if full_obs is not None and len(full_obs) > 0:
                    if self.target_bbox.has_bbox():
                        full_obs = self.target_bbox.filter_out_points(full_obs)

                # 2c. Nuage persistant du conteneur (résistant à l'occlusion)
                with self._container_cloud_lock:
                ...
                # ── 3. Effacer les voxels du robot (bras) ──
                ...
                # ── 4. Sécurité supplémentaire : nettoyage strict de la BBox cible ──
                # On force la suppression de tout obstacle dans la zone de la nourriture
                if self.target_bbox.has_bbox():
                    self.env_grid.clear_bbox(
                        self.target_bbox.bbox_min, self.target_bbox.bbox_max
                    )

                # ── 5. Mise à jour de la SDF ──
                self.perf.tick('sdf_update')
                acc_pts = self.env_grid.get_occupied_points(min_confidence=1)
                ...
                # FIX: Add min_confidence=1
                viz_pts = self.env_grid.get_occupied_points(min_confidence=1, max_points=20000)
                self._publish_markers()
                
                # FIX: Combine the full room view with the small collision grid
                if full_obs is not None and len(full_obs) > 0:
                ...
                # FIX: Throttle heavy rendering to every 3 frames
            ...
            if self._frame_count % 30 == 0:
                stages = ['sdf_loop', 'unproject', 'accumulate',
