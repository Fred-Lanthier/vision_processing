...
class SafeGenerativeNode:
    def __init__(self):
        ...
        self.num_points_pcd = cfg.get('num_points', 1024)
        ...
        self.n_steps = int(rospy.get_param("~n_steps", 10))
        self.ode_method = str(rospy.get_param("~ode_method", "midpoint")).lower()
        self.cbf_start_t = float(rospy.get_param("~cbf_start_t", 0.6))
        self.cbf_gain = float(rospy.get_param("~cbf_gain", 0.5))
    ...
    def select_planner_obstacles(self, P_obs, curr_pos):
        if P_obs is None or P_obs.shape[0] <= self.planner_cbf_max_obstacles:
            return P_obs
        with torch.no_grad():
            dist = torch.norm(P_obs - curr_pos, dim=1)
            _, idx = torch.topk(dist, k=self.planner_cbf_max_obstacles, largest=False)
            return P_obs[idx]
    ...
    def control_loop(self, event):
        ...
        try:
            ...
            if A is not None:
                if self.check_finite_debug and torch.isnan(A).any():
    ...
    def generate_safe_trajectory(self, obs_dict, curr_pos, P_obs, N_STEPS=None, T0=None, C=None, lam=1e-5):
        """Modified SAD-Flower Predictor-Corrector loop for real-time 8Hz."""
        N_STEPS = self.n_steps if N_STEPS is None else int(N_STEPS)
        T0 = self.cbf_start_t if T0 is None else float(T0)
        C = self.cbf_gain if C is None else float(C)
        ode_method = self.ode_method
        if ode_method not in ("euler", "midpoint"):
            rospy.logwarn_throttle(5, f"Unknown ode_method='{ode_method}', falling back to midpoint.")
            ode_method = "midpoint"
        B = 1
        H = 16
        dt = 1.0 / N_STEPS
        ...
        P_obs = self.select_planner_obstacles(P_obs, curr_pos)
        ...
        for i in range(N_STEPS):
            ...
            t0 = time.perf_counter()
            with torch.inference_mode():
                # Observation encoder is cached once per rollout.
                if ode_method == "euler":
                    v = self.fm_agent.velocity_net(A, t_tensor, global_cond)
                else:
                    v1 = self.fm_agent.velocity_net(A, t_tensor, global_cond)
                    A_mid = A + v1 * (dt / 2)
                    t_mid_tensor = torch.full((B,), t_val + dt / 2, device=dev)
                    v = self.fm_agent.velocity_net(A_mid, t_mid_tensor, global_cond)
            ...
            if t_val < T0 or P_obs is None:
                ...
                try:
                    ...
                    with torch.no_grad():
                        ...
                        JtJ = torch.bmm(J.transpose(1, 2), J) + lam * eye7
                        grad_h_x = torch.bmm(J, torch.linalg.solve(JtJ, grad_h_q.unsqueeze(-1))).squeeze(-1)
                        
                        # Closed-form QP in R9 (we use the 9D pose velocity for the QP)
                        v_world_9d = v_world[..., :9]
                        dot = (grad_h_x * v_world_9d).sum(-1)
                        constr = dot + phi_t * h_q
                        denom = (grad_h_x**2).sum(-1).clamp(min=1e-8)
                        lam_qp = torch.clamp(-constr / denom, min=0)

                        # --- CAUSAL FIX: Only apply CBF correction to position (x, y, z) ---
                        # Preserve the 9D rotation representation (r1, r2) generated by the FM model
                        # so that Gram-Schmidt decodes valid SE3 matrices.
                        u_corr_9d = (lam_qp.unsqueeze(-1) * grad_h_x).reshape(B, H, 9)
                        
                        u_corr_full = torch.zeros(B, H, self.action_dim, device=dev)
                        # We only want to correct the positional part (0:3)
                        u_corr_full[..., :3] = u_corr_9d[..., :3]

                        A_world_next = A_world + v_world.reshape(B, H, self.action_dim) + u_corr_full * dt
        ...
        rospy.loginfo_throttle(10, f"✅ Plan generated in {t_total*1000:.2f}ms | Stats: FM Inference={t_fm_total*1000:.2f}ms (encode={t_fm_encode*1000:.2f}ms), C++ IK={t_ik_total*1000:.2f}ms, CBF QP={t_qp_total*1000:.2f}ms")
        return A


    def publish_joint_trajectory(self, A_norm, pos_fork):
        """Publish full 16-point joint trajectory with velocities and adaptive timing."""
        A_unnorm = self.normalizer.unnormalize_act(A_norm)
        A_world = A_unnorm.clone()
        A_world[..., :3] += pos_fork
        A_world_np = A_world.squeeze(0).detach().cpu().numpy()  # [16, 27]
        ...
        # Re-normalize pose rotation columns (dims 3:9) after temporal averaging.
        # Velocity/acceleration dims (9:27) are raw derivatives — no orthogonalization needed.
        for t in range(16):
        ...
        H = len(q_traj)
        max_joint_velocity = 0.7  # rad/s — Panda joint velocity limit

        # Per-segment timing: time needed for the slowest joint to travel its displacement.
        # dt_seg[i] = max_j(|q[i+1][j] - q[i][j]|) / max_joint_velocity
        # This is the tightest physically feasible timing given the velocity limit.
        q_arr = np.array([q_traj[i][:7] for i in range(H)])
        dt_segs = []
        for i in range(H - 1):
            max_delta = np.max(np.abs(q_arr[i + 1] - q_arr[i]))
            dt_segs.append(float(np.clip(max_delta / max_joint_velocity, 0.02, 1.0)))
        dt_segs.append(dt_segs[-1] if dt_segs else 0.1)  # last segment mirrors previous

        # Cumulative timestamps
        t_stamps = np.concatenate([[0.0], np.cumsum(dt_segs[:-1])])

        # Joint velocities via central finite differences, scaled by per-segment timing.
        q_dots = []
        for i in range(H):
            if i == 0:
                q_dot = (q_arr[1] - q_arr[0]) / dt_segs[0]
            elif i == H - 1:
                q_dot = (q_arr[H - 1] - q_arr[H - 2]) / dt_segs[H - 2]
            else:
                dt_span = t_stamps[i + 1] - t_stamps[i - 1]
                q_dot = (q_arr[i + 1] - q_arr[i - 1]) / max(dt_span, 1e-6)
            q_dots.append(np.clip(q_dot, -max_joint_velocity, max_joint_velocity))
        ...
        for i in range(H):
            ...
            point.velocities = q_dots[i].tolist()
            point.time_from_start = rospy.Duration(t_stamps[i])
