...
#!/usr/bin/env python3
"""
Diffusion Policy Inference Node with Temporal Ensembling

This version implements temporal ensembling to smooth trajectories:
- Keep a buffer of recent predictions
- Average overlapping predictions before publishing
- Results in much smoother motion

Based on the original Diffusion Policy paper's recommendation.
"""
import rospy
import numpy as np
import torch
...
class TemporalEnsembler:
    ...
    Example with buffer_size=3, pred_horizon=8:
        Time 0: Store prediction [A0-A7]
        Time 1: Store prediction [B0-B7], ensemble A1 with B0
        Time 2: Store prediction [C0-C7], ensemble A2, B1, C0
        ...
    """
    ...
    def __init__(self, pred_horizon, action_dim, buffer_size=5, weights='exponential'):
        ...
        Args:
            ...
            action_dim: Dimension of each action (e.g., 9 for pos+rot6d)
        ...
        # Buffer stores (timestamp, prediction) tuples
        # Each prediction is shape (pred_horizon, action_dim)
    ...
    def add_prediction(self, prediction):
        ...
        Args:
            prediction: numpy array of shape (pred_horizon, action_dim)
        """
    ...
    def get_ensembled_action(self, future_step=0):
        ...
        Args:
            future_step: Which step into the future (0 = immediate next action)
            
        Returns:
            Ensembled action of shape (action_dim,)
        """
        ...
        for (timestamp, prediction) in self.buffer:
            ...
            # Which index in this prediction corresponds to 'future_step'?
            pred_index = age + future_step
            
            # Only use if valid index
    ...
    def get_ensembled_trajectory(self, num_steps):
        ...
        Returns:
            numpy array of shape (num_steps, action_dim)
        """
        trajectory = []
...
class DiffusionInferenceNode:
    def __init__(self):
        ...
        # --- DDIM SCHEDULER (Deterministic for less variance) ---
        self.noise_scheduler = DDIMScheduler(
            num_train_timesteps=100,
            beta_schedule='squaredcos_cap_v2',
            clip_sample=True,
        ...
        # Fewer steps = faster but noisier. More steps = slower but smoother.
        self.num_inference_steps = rospy.get_param("~num_inference_steps", 10)
        self.noise_scheduler.set_timesteps(self.num_inference_steps)

        # --- TEMPORAL ENSEMBLING ---
        # buffer_size: how many past predictions to average (more = smoother but more lag)
        # weights: 'exponential' gives more weight to recent predictions
        ...
        # How many steps of the ensembled trajectory to publish
        ...
        # Control rate (10 Hz typical for diffusion policies)
        ...
        rospy.loginfo("=" * 60)
        rospy.loginfo("🚀 Diffusion Inference Node (with Temporal Ensembling)")
    ...
    def cloud_callback(self, msg):
        ...
        # Parse based on point structure
        if point_step == 12:
            dtype_list = [('x', np.float32), ('y', np.float32), ('z', np.float32)]
        else:
    ...
    def get_robot_pose(self):
        """Get current robot pose as 9D vector (pos + rot6d)"""
        try:
            (trans, rot) = self.tf_listener.lookupTransform('/world', '/panda_hand_tcp', rospy.Time(0))
            mat = tft.quaternion_matrix(rot)[:3, :3]
            rot_6d = rotation_matrix_to_ortho6d(mat)
