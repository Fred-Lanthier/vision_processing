...
class TrajectoryFollowerContinuous:
    ...
    def process_trajectory(self, msg, frame_id):
        ...
        for i, pose_fork in enumerate(msg.poses):
            ...
            if sol is not None:
                jump = np.max(np.abs(sol - prev_joints))
                if jump <= self.max_joint_jump:
                    trajectory_joints.append(sol)
                    prev_joints = sol
                else:
                    rospy.logwarn_throttle(1.0, f"[TRAJ] Waypoint {i}: JUMP {jump:.3f} > {self.max_joint_jump}")
                    break
            else:
                rospy.logwarn_throttle(1.0, f"[TRAJ] Waypoint {i}: IK FAILED")
    ...
    def traj_callback(self, msg):
        with self.lock:
            ...
            # ── DEBUG ──
            if trajectory_joints is None:
                rospy.logwarn_throttle(1.0, "[TRAJ] IK returned NO valid waypoints!")
            else:
                rospy.loginfo_throttle(1.0, 
                    f"[TRAJ] IK OK: {len(trajectory_joints)}/{len(msg.poses)} waypoints")
            # ────────────
