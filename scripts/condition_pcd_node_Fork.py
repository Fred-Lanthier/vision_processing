    ...
    def callback(self, rgb_msg, depth_msg, joint_msg):
                ...
                if cv_depth is not None:
                    ...
                    if mask is not None and score > self.detection_confidence:
                        ...
                        if np.sum(valid) > 100:
                            ...
                            # =======================================================
                            # NOUVEAU : Filtrage de la tranche de 2.5 cm (Z caméra)
                            # =======================================================
                            min_z = np.min(z_val)
                            z_threshold = min_z + 0.025  # 2.5 cm = 0.025 m
                            
                            slice_mask = z_val <= z_threshold
                            
                            v = v[slice_mask]
                            u = u[slice_mask]
                            z_val = z_val[slice_mask]
                            
                            # Sécurité : on vérifie qu'il reste suffisamment de points
                            # après avoir coupé le fond de l'objet
                            if len(z_val) > 20:
                            # =======================================================
                                ...
                                rospy.loginfo(f"✅ TARGET DETECTED at {c} (score={score:.3f}, points conservés={len(z_val)})")
                            else:
                                rospy.logwarn("Cible détectée, mais pas assez de points dans la tranche de 2.5 cm.")
            except Exception as e:
                rospy.logwarn_throttle(5, f"Detection error: {e}")

        # --- 5. VISU, GRASP & EQUILIBRAGE DU NUAGE DE POINTS ---
        ...
        if self.cube_locked and self.static_cube_cloud is not None:
            ...
            if not self.is_grasped:
                ...
                penetration = fork_tip_pos[2] - self.static_cube_cloud_subsampled[idx_min, 2]  # Z-axis penetration
                rospy.loginfo_throttle(0.5, f"Fork→food dist: {min_dist:.4f}m with penetration={penetration:.4f}m")
                ...
                if penetration < self.contact_threshold:
                    ...
                    rospy.loginfo(f"🍴 GRASP! min_dist={min_dist:.4f}m with penetration={penetration:.4f}m")
        ...
        # --- ECHANTILLONNAGE ÉQUILIBRÉ (La clé pour Flow Matching) ---
        
        # 1. Traiter la fourchette (max 128 points)
        ...
        # 2. Traiter la nourriture (max 128 points)
        if self.cube_locked and hasattr(self, 'current_cube_cloud') and self.current_cube_cloud is not None:
        ...
        # 3. Assembler et garantir exactement 256 points
        if len(merged) > 0:
            full_cloud = np.vstack(merged)
            
            # Si on a trop de points (ex: padding imprévu), on coupe
            ...
            # Si on manque de points (ex: nourriture très petite), on pad en clonant des points existants
            ...
            # Publication du nuage propre et équilibré
